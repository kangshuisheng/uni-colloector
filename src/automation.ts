...
import { POSITION_MANAGER_ABI, ERC20_ABI } from './abis';
 ...
 */
export async function getUnclaimedFees(tokenId: bigint): Promise<Fees> {
  try {
    ...
    return {
      amount0,
      amount1,
      amount0Formatted: formatUnits(amount0, 6), // Assuming MON/AUSD decimals, need to be dynamic
      amount1Formatted: formatUnits(amount1, 18)
    };
  } catch (error) {
...
export async function rebalancePosition(
  ...
  console.log(`Rebalancing position ${tokenId} to range [${newTickLower}, ${newTickUpper}]...`);
  ...
  console.log('Rebalance complete (Minting step pending implementation details)');
